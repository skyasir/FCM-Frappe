...
_INCOME_ACCOUNT_FIELD = {
	"Rent/MG": "rent_income_account",
	"CAM": "cam_income_account",
	"Electricity": "utility_income_account",
	"Water": "utility_income_account",
	"Revenue Share": "revenue_share_income_account",
}


def _resolve_fcm_accounting(reference_contract, billing_type):
	"""Derive Company, Cost Center and (optional) Income Account from the contract's
	Food Court so every FCM invoice posts to the correct company and Food-Court-wise
	Cost Center. Each Food Court is its own Company per the business scope, so relying
	on the global default company is not safe. Returns a dict; keys are omitted when
	the corresponding master value is not configured (ERPNext defaults then apply)."""
	acc = {}
	if not reference_contract:
		return acc
	fc_name = frappe.db.get_value("FCM Lease Contract", reference_contract, "food_court")
	if not fc_name:
		return acc
	company, cost_center = frappe.db.get_value(
		"FCM Food Court", fc_name, ["company", "cost_center"]
	) or (None, None)
	if company:
		acc["company"] = company
	if cost_center:
		acc["cost_center"] = cost_center
	income_field = _INCOME_ACCOUNT_FIELD.get(billing_type)
	if income_field:
		income_account = frappe.db.get_value("FCM Food Court", fc_name, income_field)
		if income_account:
			acc["income_account"] = income_account
	return acc
...
def _make_sales_invoice(customer, items, due_days, reference_contract, billing_type):
	...
	Company, Cost Center and Income Account are resolved from the contract's Food Court
	so invoices always post to the correct company and Food-Court-wise Cost Center."""
	acc = _resolve_fcm_accounting(reference_contract, billing_type)
	...
	if acc.get("company"):
		si.company = acc["company"]
	if acc.get("cost_center"):
		si.cost_center = acc["cost_center"]
	for row in items:
		# apply Food-Court-wise cost center and income account per line where available
		if acc.get("cost_center"):
			row.setdefault("cost_center", acc["cost_center"])
		if acc.get("income_account"):
			row.setdefault("income_account", acc["income_account"])
		si.append("items", row)
